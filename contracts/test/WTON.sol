...
contract SeigToken is ERC20, Ownable, ERC20OnApprove, AuthController {
  ...
  function setSeigManager(SeigManagerI _seigManager) external virtual onlyOwner {
...
contract WTON is DSMath, ReentrancyGuard, Ownable, MinterRole, ERC20Detailed, SeigToken, OnApprove
{
   using SafeERC20 for ERC20Mintable;
   using Roles for ERC20Mintable;

  ERC20Mintable public ton;

  constructor (ERC20Mintable _ton) ERC20Detailed("Wrapped TON", "WTON", 27)
  {
  ...
  function _swapToTON(address tonAccount, address wtonAccount, uint256 wtonAmount) internal returns (bool) {
    super._burn(wtonAccount, wtonAmount);
  ...
  function _swapFromTON(address tonAccount, address wtonAccount, uint256 tonAmount) internal returns (bool) {
    super._mint(wtonAccount, _toRAY(tonAmount));
